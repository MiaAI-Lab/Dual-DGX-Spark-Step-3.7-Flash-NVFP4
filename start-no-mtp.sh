...
NNODES="${NNODES:-2}"
make_node_script "$REPO_ROOT/templates/launch-no-mtp.sh" "$HEAD_SCRIPT" "$NNODES" 0 "$HEAD_IP"
make_node_script "$REPO_ROOT/templates/launch-no-mtp.sh" "$WORKER_SCRIPT" "$NNODES" 1 "$HEAD_IP"
...
LOGFILE="$REPO_ROOT/logs/start-no-mtp-$(date +%Y%m%d-%H%M%S).log"
...
LOGS_PID=$!
echo "$LOGS_PID" > "$REPO_ROOT/logs/start-no-mtp.pid"
